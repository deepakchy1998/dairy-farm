...
router.put('/profile', auth, validate(updateProfileSchema), async (req, res, next) => {
  try {
    ...
    if (req.body.chatBubbleEnabled !== undefined) user.chatBubbleEnabled = !!req.body.chatBubbleEnabled;
    await user.save();
    const userData = { _id: user._id, name: user.name, email: user.email, phone: user.phone, role: user.role, farmId: user.farmId, profilePhoto: user.profilePhoto, farmEnabled: user.farmEnabled, chatBubbleEnabled: user.chatBubbleEnabled, createdAt: user.createdAt };
