...
router.get('/', async (req, res, next) => {
  try {
    ...
    const [data, total] = await Promise.all([
      Revenue.find(filter).sort('-date').skip(p.skip).limit(p.limit).populate('cattleId', 'tagNumber breed category gender weight').lean(),
...
router.put('/:id', async (req, res, next) => {
  try {
    const record = await Revenue.findOneAndUpdate({ _id: req.params.id, farmId: req.user.farmId }, req.body, { new: true, runValidators: true });
...
router.delete('/:id', async (req, res, next) => {
  try {
    const record = await Revenue.findOneAndDelete({ _id: req.params.id, farmId: req.user.farmId });
    if (!record) return res.status(404).json({ success: false, message: 'Not found' });
